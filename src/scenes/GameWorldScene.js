...
export default class GameWorldScene extends Phaser.Scene {
  ...
  ensureCriticalAssets() {
    ...
    criticalAssets.forEach((assetKey) => {
      if (!this.textures.exists(assetKey)) {
        console.warn(
          `Critical asset not found: ${assetKey}, creating fallback`
        );
  ...
  create() {
    // Connect systems to this scene
    const gameManager = this.plugins.get('GameManager');
    if (gameManager) {
      gameManager.connectSystemsToScene(this);
    }
    ...
    for (let y = 0; y < this.worldHeight; y++) {
      ...
      for (let x = 0; x < this.worldWidth; x++) {
        ...
        if (
          ...
          if (
            (x === 12 && y === 0) || // North transition
            (x === this.worldWidth - 1 && y === 9) || // East transition
            (x === 12 && y === this.worldHeight - 1)
          ) {
            // South transition
            world[y][x] = "grass";
          } else {
  ...
  showSaveLoadNotification(message, color = 0x27ae60) {
    const notification = this.add.text(
      ...
      message,
      {
        ...
        fontFamily: "Arial",
        color: "#ffffff",
        backgroundColor: `rgba(${(color >> 16) & 255}, ${(color >> 8) & 255}, ${
          color & 255
        }, 0.9)`,
        padding: { x: 15, y: 8 },
      }
  ...
  checkAreaTransitions() {
    ...
    this.transitionZones.children.entries.forEach((zone) => {
      ...
      if (playerGridX === zoneGridX && playerGridY === zoneGridY) {
        this.triggerAreaTransition(
          zone.getData("direction"),
          zone.getData("targetArea")
        );
      }
    });
  }
  ...
  setupInventoryEventListeners() {
    ...
    // Use the InventorySystem's addEventListener method
  ...
  setupPowerEventListeners() {
    ...
    // Use the PowerSystem's addEventListener method
  ...
  setupStoryEventListeners() {
    ...
    // Use the StorySystem's addEventListener method
  ...
  addStoryTestControls() {
    ...
    const testInstructions = this.add.text(
      10,
      180,
  ...
  handleStoryTestControls() {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.testKeys.D)) {
      ...
      const randomDialogue =
        dialogueEvents[Math.floor(Math.random() * dialogueEvents.length)];
  ...
  initializeParticleEffects() {
    ...
    this.particleEffects.powerActivation = this.add.particles(
      0,
      0,
      "grass-tile",
      {
    ...
    this.particleEffects.itemCollection = this.add.particles(
      0,
      0,
      "grass-tile",
      {
    ...
    this.particleEffects.ambientSparkles = this.add.particles(
      0,
      0,
      "grass-tile",
      {
  ...
  updateParticleEffects(time, delta) {
    ...
    if (this.player && this.player.isMoving && this.player.isMoving()) {
      ...
      if (time % 200 < delta) {
        // Every 200ms
        this.particleEffects.footsteps.setPosition(
          playerPos.x,
          playerPos.y + 16
        );
  ...
  triggerPowerParticles(x, y, powerType = "default") {
    ...
    this.particleEffects.powerActivation.setTint(
      powerColors[powerType] || powerColors.default
    );
  ...
  addPowerUI() {
    ...
    const powerInstructions = this.add.text(
      10,
      350,
      "Power Controls:\nQ - Telekinesis\nE - Enhanced Vision\nR - Time Slow\nF - Phase Walk",
      {
  ...
  updatePowerUI() {
    ...
    if (unlockedPowers.length === 0) {
      ...
      this.powerListText.setText(["Unlocked Powers:", ...powerTexts]);
    }
  }
  ...
  isCollisionTile(gridX, gridY) {
    ...
    if (
      gridX < 0 ||
      gridX >= this.worldWidth ||
      gridY < 0 ||
      gridY >= this.worldHeight
    ) {
  ...
  addInventoryUI() {
    ...
    const inventoryInstructions = this.add.text(
      10,
      480,
  ...
  addInventoryTestControls() {
    ...
    // Add test instructions to UI
    const inventoryTestInstructions = this.add.text(
      10,
      520,
  ...
  spawnEnemies() {
    ...
    spawnPoints.forEach((spawn) => {
      ...
      // Add the enemy sprite to the group, not the enemy object itself
      if (enemy.sprite) {
        this.enemies.add(enemy.sprite);
        // Store reference to the enemy object on the sprite for easy access
  ...
  updateCombatSystem(time, delta) {
    ...
    this.enemies.children.entries.forEach((enemySprite) => {
      if (enemySprite.enemyData && enemySprite.enemyData.update) {
        enemySprite.enemyData.update(time, delta);
      }
    });
   ...
   */
  startCombat(enemySprite) {
    const enemy = enemySprite.enemyData;
    if (this.combatActive || !enemy || enemy.isDefeated) return;
  ...
  onEnemyDefeated(enemy) {
    ...
    const remainingEnemies = this.enemies.children.entries.filter(
      (e) => !e.isDefeated
    );
